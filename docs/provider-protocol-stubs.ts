...
export interface ProviderProtocol {
   ...
   * autoload(env) { return false }
   *
   * @example
   * // Amazon Bedrock: auto-load if AWS credentials present
   * autoload(env) {
   ...
   *
   * @example
   * // OpenAI: route to responses API
   * getModel(sdk, modelID) {
   *   return sdk.responses(modelID)
   * }
   *
   * @example
   * // GitHub Copilot: conditional routing
   * getModel(sdk, modelID) {
   ...
   * @example
   * // Anthropic
   * wrapOptions(opts) { return { anthropic: opts } }
   *
   * @example
   * // OpenAI-compatible providers
   * wrapOptions(opts) { return { openai: opts } }
   */
   ...
   *
   * @example
   * // Google/Gemini: convert integer enums to string
   * transformSchema(schema) {
   *   return this.convertIntegerEnumsToString(schema)
   * }
   */
   ...
   *
   * @example
   * // GitHub Copilot: add settings link
   * transformError(error) {
   *   if (error.message.includes('model not supported')) {
   *     return `${error.message}\n\nEnable model at: https://github.com/settings/copilot`
   *   }
   *   return error.message
   * }
   */
...
export interface AuthMethod {
  /** Method type */
  type: "api_key" | "oauth" | "adc" | "aws_chain" | "custom";
...
export class AuthenticationError extends Error {
  constructor(
    public readonly providerID: string,
    public readonly method: AuthenticationType,
    public readonly reason: string
  ) {
...
// =============================================================================
// SECTION 13: USAGE TRACKING INTERFACE
// =============================================================================

/**
 * Tracks provider usage for analytics, billing, and routing optimization.
 */
export interface UsageTracker {
   ...
   */
  getProviderStats(providerID: string, timeRange?: TimeRange): ProviderUsageStats;
   ...
   */
  getAggregateStats(timeRange?: TimeRange): AggregateUsageStats;
}
...
export interface UsageRecord {
  ...
  /** Original provider if fallback used */
...
export interface SessionUsageStats {
  ...
  /** Provider breakdown */
  byProvider: Map<string, ProviderSessionStats>;
}

/** Provider statistics within a session */
...
export interface AggregateUsageStats {
  ...
  /** By task type */
  byTask: Map<TaskType, TaskUsageStats>;
}
...
// =============================================================================
// SECTION 14: HEALTH MONITORING INTERFACE
// =============================================================================

/**
 * Monitors provider health for routing decisions.
 */
export interface HealthMonitor {
   ...
   */
  check(providerID: string): Promise<HealthStatus>;
   ...
   *
   * @returns Array of healthy provider IDs
   */
   ...
   *
   * @param listener - Callback for health changes
...
export interface HealthStatus {
  ...
  /** Current latency (rolling average) */
  latencyMs?: number;
...
export type HealthStatusListener = (
  providerID: string,
...
// =============================================================================
// SECTION 15: BASE PROVIDER IMPLEMENTATION
// =============================================================================

/**
 * Abstract base class with default implementations.
 * Providers can extend this to reduce boilerplate.
 */
export abstract class BaseProvider implements ProviderProtocol {
  ...
  async initializeSDK(credentials: Credentials, options: SDKOptions): Promise<ProviderSDK> {
    // Default: dynamic import of SDK package
    const sdkModule = await import(this.sdk);
    const createSDK = sdkModule.default || sdkModule[`create${this.name}`];
    return createSDK({
  ...
  getModel(sdk: ProviderSDK, modelID: string, _options?: ModelOptions): LanguageModelV2 {
    // Default: use languageModel method
    if (sdk.languageModel) {
  ...
  filterUnsupportedParts(parts: ContentPart[], model: Model): ContentPart[] {
    const supportedModalities = new Set(model.modalities.input);
    return parts.filter((part) => {
      const modality = this.getPartModality(part);
      return supportedModalities.has(modality);
    });
  }
  ...
  buildOptions(ctx: OptionsContext): ProviderOptions {
    ...
    if (ctx.variant) {
      const variantConfig = this.variants(ctx.model).find((v) => v.name === ctx.variant);
      if (variantConfig) {
  ...
  buildSmallModelOptions(_model: Model): ProviderOptions {
    // Default: minimal options for fast execution
    return {
  ...
  protected sanitizeToolCallId(id: string, pattern: RegExp, replacement: string): string {
