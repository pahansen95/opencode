/**
 * Cohere Provider Implementation
 *
 * Integrates Cohere models via standard SDK.
 *
 * @module provider/providers/cohere
 */

import { BaseProvider } from "../base"
import type {
  AuthContext,
  Credentials,
  ProviderCapabilities,
  ProviderOptions,
  NamespacedOptions,
  ListModelsContext,
  Model,
} from "../protocol"

/**
 * Cohere provider.
 *
 * Key behaviors:
 * - Standard bundled SDK integration
 * - No reasoning variants
 */
export class CohereProvider extends BaseProvider {
  override readonly id = "cohere"
  override readonly sdk = "@ai-sdk/cohere"
  override readonly name = "Cohere"
  override readonly url = "https://cohere.com"

  /**
   * Resolve credentials from env or auth store.
   */
  override async authenticate(ctx: AuthContext): Promise<Credentials | null> {
    // Environment variable
    const apiKey = ctx.env.COHERE_API_KEY ?? ctx.env.CO_API_KEY
    if (apiKey) return { apiKey }

    // Auth store
    const stored = await ctx.authStore.get("cohere")
    if (stored?.apiKey) return { apiKey: stored.apiKey }

    return null
  }

  /**
   * Models are fetched from models.dev.
   */
  override async listModels(_ctx: ListModelsContext): Promise<Model[]> {
    return []
  }

  /**
   * Wrap options in Cohere namespace.
   */
  override wrapOptions(options: ProviderOptions): NamespacedOptions {
    return { cohere: options }
  }

  /**
   * Declare capabilities.
   */
  override capabilities(): ProviderCapabilities {
    return {
      caching: false,
      reasoning: "none",
      modalities: { input: ["text"], output: ["text"] },
      authentication: ["api_key"],
      streaming: true,
      toolCalling: true,
      structuredOutput: true,
    }
  }
}
